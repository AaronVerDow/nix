{
  ...
  outputs = { self, nixpkgs, home-manager, nixos-hardware, solaar, ... }@inputs:
    let
    ...
    in {
      ...
      nixosConfigurations = {
        ark = nixpkgs.lib.nixosSystem {
          ...
          modules = [ ./hosts/ark/configuration.nix ./modules/nixos/kanata ];
        };
        ...
        smartboard = nixpkgs.lib.nixosSystem {
          specialArgs = { inherit inputs outputs; };
          modules =
            [ ./hosts/smartboard/configuration.nix ./modules/nixos/kanata ];
        };
      };
